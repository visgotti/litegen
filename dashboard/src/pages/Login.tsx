...
function GithubButton({ testId, label }: { testId: string; label: string }) {
  return (
    <button type="button" data-testid={testId} onClick={() => oauthStart('github')} style={providerBtnStyle}>
...
function GoogleButton({ testId, label }: { testId: string; label: string }) {
  return (
    <button type="button" data-testid={testId} onClick={() => oauthStart('google')} style={providerBtnStyle}>
...
export default function Login() {
  ...
  useEffect(() => {
    ...
    client.auth
      ...
      .catch(() => {
        // If the config endpoint is unavailable, fall back to showing the
        // password form + both OAuth buttons (dev / single-tenant default).
    ...
    try {
      ...
      if (err instanceof LiteGenAPIError) {
        if (err.status === 401) {
          ...
          const retryAfter = err.code ?? '';
          setError(`Too many attempts. Please try again later.${retryAfter ? ` (retry after ${retryAfter}s)` : ''}`);
        } else {
      ...
      <div className="auth-card">
        ...
        {passwordEnabled && (
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div>
              <label style={{ display: 'block', marginBottom: 6, color: '#8b949e', fontSize: 13 }}>Password</label>
              <input
                className="input"
        ...
        {(githubEnabled || googleEnabled) && (
          <div style={{ margin: passwordEnabled ? '24px 0 0' : 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {githubEnabled && <GithubButton testId="oauth-github" label="Sign in with GitHub" />}
            {googleEnabled && <GoogleButton testId="oauth-google" label="Sign in with Google" />}
          </div>
        )}
