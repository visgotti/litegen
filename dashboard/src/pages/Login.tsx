...
export default function Login() {
  ...
  useEffect(() => {
    // Try to detect which OAuth providers are enabled.
    // If the backend doesn't expose a config endpoint, default to showing both.
    setAuthConfig({ providers: ['github', 'google'] });
  }, []);
  ...
  const githubEnabled = authConfig?.providers?.includes('github') ?? true;
  const googleEnabled = authConfig?.providers?.includes('google') ?? true;
    ...
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', background: '#0d1117' }}>
      <div style={{ width: 380, padding: 32, background: '#161b22', borderRadius: 12, border: '1px solid #30363d' }}>
        ...
        {(githubEnabled || googleEnabled) && (
          <div style={{ margin: '24px 0 0', display: 'flex', flexDirection: 'column', gap: 10 }}>
