...
/** @typedef {{ image: boolean, video: boolean, model3d: boolean }} OutputModalities */
...
export function deriveCapabilities(models) {
  /** @type {Map<string, {text:boolean,image:boolean,multi:boolean,hasImage:boolean,hasVideo:boolean,has3d:boolean}>} */
  const acc = new Map();
  ...
  for (const [provider, v] of acc) {
    ...
    const anyOut = v.hasImage || v.hasVideo || v.has3d; // never emit a modality-less node
    out[provider] = {
      provider,
...
export function renderGeneratedTs(caps) {
  ...
  const entries = ids.map((id) => {
    ...
    const i = c.inputs;
    const o = c.outputs;
    return `  ${JSON.stringify(id)}: { provider: ${JSON.stringify(c.provider)}, inputs: { text: ${i.text}, image: ${i.image}, multi: ${i.multi} }, outputs: { image: ${o.image}, video: ${o.video}, model3d: ${o.model3d} } },`;
  });
