...
| `guidance_scale` | Float | range | 12 | validator: `float_out_of_range`. provider: `fal::tests::generates_flux_dev_image` (`body["guidance_scale"]==3.5`), `replicate::tests::generates_flux_dev_via_polling` (`body["input"]["guidance_scale"]==3.5`). conformance: n/a (not enum). | ✅ |
